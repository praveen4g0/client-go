...
func (c *controller) processItem() bool {
	...
	if apierrors.IsNotFound(err) {
		// delete services and Ingress controller tied to owner references of deployment
	...
	err = c.LinkServicesToDeployment(ns, name)
	if err != nil {
		// re-try
...
func (c *controller) LinkServicesToDeployment(namespace, name string) error {
	ctx := context.Background()
	...
	svc := &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			Selector: dep.Spec.Template.Annotations,
	...
	s, err := c.clientset.CoreV1().Services(namespace).Create(ctx, svc, metav1.CreateOptions{})
	if err != nil {
